...
function db_schema_mysql($pdo) {
    ...
    $pdo->exec("CREATE TABLE IF NOT EXISTS cotacao (
        id INT NOT NULL AUTO_INCREMENT, obra_id INT, servico_id INT, titulo VARCHAR(255) NOT NULL, apelido VARCHAR(160), colaboradores TEXT,
    ...
    $pdo->exec("CREATE TABLE IF NOT EXISTS neg_grupo (
        ...
        categoria VARCHAR(20) DEFAULT 'material', modo VARCHAR(16) DEFAULT 'consumo',
    ...
    try {
        ...
        if ($cc && !isset($cc['colaboradores'])) $pdo->exec("ALTER TABLE cotacao ADD COLUMN colaboradores TEXT");   // JSON de bitrix_ids que também podem editar (compartilhar — férias do criador)
        // trilha de auditoria POR COTAÇÃO (quem mudou o quê, quando) — compartilhamento exige transparência
        ...
        // REVISÕES de proposta (não perder o histórico quando o fornecedor manda preço novo): revisao 0,1,2…;
        // raiz_id = proposta original da cadeia; ativa=1 só na revisão vigente (a que entra no mapa comparativo)
...
function db_schema($pdo) {
    ...
    $pdo->exec("CREATE TABLE IF NOT EXISTS neg_grupo (id INTEGER PRIMARY KEY AUTOINCREMENT, nome TEXT, ordem INTEGER DEFAULT 0, categoria TEXT DEFAULT 'material', modo TEXT DEFAULT 'consumo', servicos TEXT, nota TEXT, updated_by TEXT, updated_at TEXT)");
