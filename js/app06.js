...
function userForm(bid){
  ...
  const pc=u?u.perm_crono:0, po=u?u.perm_orcamento:0, pq=u?u.perm_quant:0, pd=u?u.perm_dicionario:0, pr=u?u.perm_responsaveis:0, pe=u?u.perm_email:0, pw=u?u.perm_whats:0;
...
function userPreset(){
  ...
  ['pCrono','pOrc','pQuant','pDic','pRespLote','pEmail','pWhats'].forEach(id=>{const e=document.getElementById(id); if(e)e.checked=false;}); // presets definidos zeram as específicas
...
async function userLoteSave(){
  ...
  if(ck('ltApPerms')){ campos.perm_crono=ck('lp-crono')?1:0; campos.perm_orcamento=ck('lp-orc')?1:0; campos.perm_quant=ck('lp-quant')?1:0; campos.perm_dicionario=ck('lp-dic')?1:0; campos.perm_responsaveis=ck('lp-resp')?1:0; campos.perm_email=ck('lp-email')?1:0; campos.perm_whats=ck('lp-whats')?1:0; }
